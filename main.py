...
def main():
    ...
    crossbar_size = hardware_parameter_list['Crossbar_size']
    ...
    for i in range(0,len(network_parameter_list)):
        if network_parameter_list[i]['type'] == 'convolution':
            ...
            if dataflow == "row_stationary" or dataflow == "output_stationary":
        ...
        if network_parameter_list[i]['type'] == 'fully_connected':
            # Layer info of workload
            Zrate = network_parameter_list[i]['zero_rate']
            # Input feature map
            x = network_parameter_list[i]['in_neuron']
            y = network_parameter_list[i]['out_neuron']
    ...
    print("The energy of crossbar computation is ", '{:.2e}'.format(crossbar_energy), "J")
